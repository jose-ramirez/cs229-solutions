...
Xtest = full(Xtest);
m_test = numTestDocs;
K_test = zeros(m_test, m_train);
M1 = Xtest * Xtrain';
train_norms = sum(Xtrain.^2, 2);
test_norms = sum(Xtest.^2, 2);
K_test = exp(-(train_norms' + test_norms - (2 * M1)) / (2 * (tau ** 2)));
predictions = K_test * average_alpha;
%---------------


% Compute the error on the test set
test_error = sum(ytest .* predictions <= 0) / numTestDocs;
